...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type="50d"):
    ...
    for word in words:
        ...
        if target_word in word_index_dict:
            idx = word_index_dict[target_word]
            
            embedding += embeddings[idx] 
            valid_word_count += 1    

    # Only use the valid word instead of len(words)
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    categories = st.session_state.categories.split(" ")
    cosine_sim = {}
    user_input = st.session_state.get('text_search', "")
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        input_emb = averaged_glove_embeddings_gdrive(user_input, word_index_dict, embeddings, model_type)
        
        for i, category in enumerate(categories):
            cat_emb = averaged_glove_embeddings_gdrive(category, word_index_dict, embeddings, model_type)
            cosine_sim[i] = cosine_similarity(input_emb, cat_emb)
        ##########################################
        ...
        input_emb = get_openai_embeddings(user_input, model_name)
        
        for i, category in enumerate(categories):
            cat_emb = st.session_state[cache_key][category]
            cosine_sim[i] = cosine_similarity(input_emb, cat_emb)
        ##########################################
        ...
        input_emb = get_sentence_transformer_embeddings(user_input, model_name)
    ...
    sorted_results = sorted(cosine_sim.items(), key=lambda x: x[1], reverse=True)
    ##########################################
